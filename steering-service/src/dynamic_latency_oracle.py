...
class DynamicLatencyOracle:
    ...
    # Defina aqui as coordenadas padrão que você quer que o oráculo use inicialmente.
    # É recomendado que estas correspondam aos valores padrão na UI (Content Steering.html).
    DEFAULT_INITIAL_CLIENT_LAT: float = -23.0 # Exemplo, ajuste conforme sua UI
    DEFAULT_INITIAL_CLIENT_LON: float = -47.0 # Exemplo, ajuste conforme sua UI
    ...
    def __init__(self, monitor, update_interval_seconds: int = 2):
        ...
        self.server_base_latencies_config = {
            "video-streaming-cache-1": 30, # Latência base atualizada
            "video-streaming-cache-2": 25, # Latência base atualizada
            "video-streaming-cache-3": 50  # Latência base atualizada
        }
        self.server_geo_coords = {}
        # Inicializa as coordenadas do cliente com os valores padrão definidos acima
